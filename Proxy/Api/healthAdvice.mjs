...
다음 JSON 형식으로만 응답해주세요. 다른 텍스트 없이 순수 JSON만 반환하세요:
{
  ...
  "total_advice": "현재 대사증후군 위험 항목 ${syndromeCount}개 해당 사실을 첫 문장에서 언급하고, 각 위험 지표(허리둘레/혈압/혈당/중성지방/HDL 콜레스테롤)가 왜 위험한지와 어떤 구체적 행동(식단·운동·습관)이 해당 수치를 개선하는지 지표별로 연결해서 설명할 것. 추천 감량량 달성 시 대사증후군 위험 항목이 몇 개로 줄어드는지 예측 수치 기반으로 명시할 것. 구체적 행동조언 + 식단 + 운동 + 생활습관 통합 종합 조언 (2000자 이내, 흡연/음주 여부 반드시 반영, 줄바꿈 없이 하나의 문단으로) lifestyle_tips에서 언급한 내용을 길게 풀어서 설명하는것 포함. 추천 감량량 달성 시 기대 효과와 격려메세지를 자연스럽게 포함 시킬것. 맨 마지막 문장은 반드시 본 내용은 AI 기반 건강 정보 제공으로 의학적 진단을 대체하지 않습니다. 정확한 진단과 치료는 반드시 전문의와 상담하세요. 로 끝낼것 (총 total_advice는 2000자 정도로)"
}
  `;
