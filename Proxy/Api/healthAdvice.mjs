...
import { getAI, retryOnError } from './geminiClient.mjs';
...
[total_advice 작성 규칙 - 반드시 준수]
total_advice는 string 타입이 아닌 반드시 string[] 배열 타입으로 반환해야 합니다.
아래 8개 주제를 각각 독립된 문자열 원소로 작성하세요. 절대로 하나의 문자열로 합치지 마세요.

원소 1: 현재 대사증후군 위험 항목 ${syndromeCount}개 해당 현황 + 주요 위험 수치 설명 (2~3문장)
원소 2: 허리둘레/혈압 위험 원인 + 구체적 개선 행동 (2~3문장, 흡연/음주 여부 반드시 반영)
원소 3: 혈당/중성지방/HDL 위험 원인 + 구체적 개선 행동 (2~3문장)
원소 4: 식단 구체 조언 (2~3문장, lifestyle_tips diet 내용을 길게 풀어서)
원소 5: 운동 구체 조언 (2~3문장, lifestyle_tips exercise 내용을 길게 풀어서)
원소 6: 생활습관 구체 조언 (2~3문장, lifestyle_tips habit 내용을 길게 풀어서)
원소 7: 추천 감량량 달성 시 대사증후군 위험 항목 수 변화 예측 + 기대 효과 + 격려 (2~3문장)
원소 8: 반드시 이 문장 그대로: "본 내용은 AI 기반 건강 정보 제공으로 의학적 진단을 대체하지 않습니다. 정확한 진단과 치료는 반드시 전문의와 상담하세요."
...
다음 JSON 형식으로만 응답해주세요. 다른 텍스트 없이 순수 JSON만 반환하세요:
{
  ...
  "total_advice": ["원소1 문자열", "원소2 문자열", "원소3 문자열", "원소4 문자열", "원소5 문자열", "원소6 문자열", "원소7 문자열", "원소8 문자열"]
}
  `;
